...
def flagfinalCallback(msg):
    global cross_flag
    if msg.data == 1:
	cross_flag=0
	cross_start_time=time.time()
	print("the car is started")
def hilensCallback(msg):
    global cross_flag
    if msg.data==9 or msg.data==10:
	cross_flag = 1
	print("detect banmaxian")

rospy.Subscriber("laser_direction", Twist, lasercallback)
rospy.Subscriber("/flag_final",Int32,flagfinalCallback)
rospy.Subscriber("/hilens", Int32,hilensCallback)
...
def cam():
    global cap, M, m_inv, point_color, thickness,laser_flag,pre_dire,pre_pre_dire
    global pre,pre_pre,start_time,cross_flag,cross_start_time
...
#    print("0st")
	...
	if cross_flag==1 or (cross_start_time!=0 and (time.time()-cross_start_time)<1):
	    print ("in the cross area")
	    if pre_dire == -1 and pre_pre_dire == -1:
	        lane_base=lane_base2
	        print ("use lane_base2")
