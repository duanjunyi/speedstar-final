...
class laneDetect:
    def __init__(self, Mwarp, camMat, camDistortion, kerSz, grayThr, frameHeight, frameWidth, roiXRatio, window_width,
                 nwindows, minpix, x_cmPerPixel, y_cmPerPixel, roadWidth, isShow):
        ...
        self.grayThr = grayThr  # 二值化阈值
        self.frame_h = frameHeight  # 窗高
        self.frame_w = frameWidth  # 窗宽
        self.roiXRatio = roiXRatio  # 统计x方向上histogram时选取的y轴坐标范围，以下方底边为起始点，比例定义终止位置
    ...
    def spin(self, img):
        ...
        if l_win_nums>=self.wins_thr and r_win_nums>=self.wins_thr:
            ...
            self.lane_cmPerPixel = self.roadWidth / np.abs(rx - lx)
            distance_from_center = (veh_pos - cen_pos) * self.lane_cmPerPixel
        elif l_win_nums >= r_win_nums and l_win_nums>0:  # 只检出左车道线
            cen_pos = np.polyval(self.lane_curve[0], self.frame_h)*self.lane_cmPerPixel + self.roadWidth / 2  # 车道中心线位置
            veh_pos = self.frame_w / 2.0  * self.lane_cmPerPixel # 小车位置，目前定义为画面中心
            distance_from_center = veh_pos - cen_pos
        elif r_win_nums > l_win_nums:   # 只检出右车道线
            cen_pos = np.polyval(self.lane_curve[1], self.frame_h)*self.lane_cmPerPixel - self.roadWidth / 2  # 车道中心线位置
            veh_pos = self.frame_w / 2.0  * self.lane_cmPerPixel # 小车位置，目前定义为画面中心
            distance_from_center = veh_pos - cen_pos
        ...
        if self.show:
            ...
            if  l_win_nums >= self.wins_thr and r_win_nums >= self.wins_thr:
                ...
                # 计算曲率
                l_curve_real = np.polyfit(y * self.y_cmPerPixel, l_x * self.x_cmPerPixel, 2)  # 映射到现实尺度下左车道线的拟合
                r_curve_real = np.polyfit(y * self.y_cmPerPixel, r_x * self.x_cmPerPixel, 2)  # 映射到现实尺度下右车道线的拟合
